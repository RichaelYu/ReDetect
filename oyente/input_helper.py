...
class InputHelper:
    ...
    def _compile_solidity(self):
        try:
            ...
            com = CryticCompile(self.source, solc_remaps=self.remap, solc_args=' '.join(options))   #编译
            contracts = self._extract_bin_obj(com)  #extract contract

            libs = com.contracts_names.difference(com.contracts_names_without_libraries)#using libs in contract
            if libs:
