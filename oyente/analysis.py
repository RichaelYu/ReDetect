...
def check_unexpected_ether_bug(path_conditions_and_vars, stack, global_state):
    path_condition = path_conditions_and_vars["path_condition"]
    ret_val = False
    # gas_call = stack[0]
    # if not isSymbolic(gas_call) or  is_expr(gas_call):
    #     ret_val = True
    if path_condition:
        list_vars = get_vars(path_condition[-1])
        for var in list_vars:
            if var.decl().name().startwith('balance'):
                ret_val = True
                break

    return ret_val

def check_gas_hardcoded_bug(path_conditions_and_vars, stack, global_state):
    gas_call = stack[0]
    ret_val = False
    if is_expr(gas_call):
        list_vars = get_vars(gas_call)
        for var in list_vars:
            if var.decl().name().startwith('amount'):
                ret_val = True
                break
    if not isSymbolic(gas_call) and gas_call != 0:
        ret_val = True
    return ret_val
...
def check_reentrancy_bug(path_conditions_and_vars, stack, global_state):
    ...
    solver.add(stack[2] > global_state["value"])
    solver.add(stack[2] > 0)
    if is_expr((stack[2])):
        list_vars = get_vars(stack[2])
        for var in list_vars:
            solver.add(var >= 0)
    ...
    for expr in path_condition:
        ...
        IsStorage = False
        for var in list_vars:
            if var == global_state["sender_address"]:
                IsAddr = True
            # check if a var is global
            if is_storage_var(var):
                IsStorage = True
            if IsAddr and IsStorage:
                ret_val = False
                return ret_val
    ...
    if is_expr(stack[2]):
        list_vars = get_vars(stack[2])
        for var in list_vars:
            if global_state["balance"]["Ia"] == var:
                ret_val = False
                return ret_val
    ...
    for expr in path_condition:
        ...
        for var in list_vars:
            if var == global_state["sender_address"]:
                IsAddr = True
            # check if a var is global
            if var == global_state['receiver_address']:
                IsStorage = True
            if IsAddr and IsStorage:
                ret_val = False
                return ret_val
...
def update_analysis(analysis, opcode, stack, mem, global_state, path_conditions_and_vars, solver):
    ...
    if opcode == "CALL":
        ...
        gas_hardcoded_result = check_gas_hardcoded_bug(path_conditions_and_vars, stack, global_state)
        analysis["gas_hardcoded_bug"].append(gas_hardcoded_result)
