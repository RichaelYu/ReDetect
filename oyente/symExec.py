...
from vulnerability import CallStack, TimeDependency, MoneyConcurrency, Reentrancy, AssertionFailure, ParityMultisigBug2, IntegerUnderflow, IntegerOverflow, GasHardcoded, UnexpectedEther
...
def initGlobalVars():
    ...
    global gas_hardcoded_all_paths
    gas_hardcoded_all_paths = []

    global unexpected_ether_all_paths
    unexpected_ether_all_paths = []
    ...
    global_problematic_pcs = {"reentrancy_bug": [], "money_concurrency_bug": [], "time_dependency_bug": [],
                              "assertion_failure": [], "integer_overflow": [], "integer_underflow": [],
                              "gas_hardcoded_bug": [], "unexpected_ether_bug": []}
...
def sym_exec_block(params, block, pre_block, depth, func_call, current_func_name):
    ...
    gas_hardcoded_all_paths.append((analysis["gas_hardcoded_bug"]))
    unexpected_ether_all_paths.append(analysis["unexpected_ether_bug"])
...
def sym_exec_ins(params, block, instr, func_call, current_func_name):
    ...
    if opcode == "INVALID" or opcode == "REVERT":
        unexpected_ether_result = check_unexpected_ether_bug(path_conditions_and_vars, stack, global_state)
        analysis["unexpected_ether_bug"].append(unexpected_ether_result)
        if analysis["unexpected_ether_bug"] and analysis["unexpected_ether_bug"][-1]:
            global_problematic_pcs["unexpected_ether_bug"].append(global_state["pc"])
    if opcode == "INVALID":
        return
    ...
    if opcode == "CALL" and analysis["gas_hardcoded_bug"] and analysis["gas_hardcoded_bug"][-1]:
        global_problematic_pcs["gas_hardcoded_bug"].append(global_state["pc"])
...
def detect_gas_hardcoded():
    global g_src_map
    global results
    global gas_hardcoded

    pcs = global_problematic_pcs["gas_hardcoded_bug"]
    gas_hardcoded = GasHardcoded(g_src_map, pcs)

    if g_src_map:
        results['vulnerabilities']['gas_hardcoded'] = gas_hardcoded.get_warnings()
    else:
        results['vulnerabilities']['gas_hardcoded'] = gas_hardcoded.is_vulnerable()
    log.info("\t  Gas Hardcoded Vulnerability: \t\t %s", gas_hardcoded.is_vulnerable())

def detect_unexpected_ether():
    global g_src_map
    global results
    global unexpected_ether

    pcs = global_problematic_pcs["unexpected_ether_bug"]
    unexpected_ether = UnexpectedEther(g_src_map, pcs)

    if g_src_map:
        results['vulnerabilities']['unexpected_ether'] = unexpected_ether.get_warnings()
    else:
        results['vulnerabilities']['unexpected_ether'] = unexpected_ether.is_vulnerable()
    log.info("\t  Unexpected_ether Vulnerability: \t\t %s", unexpected_ether.is_vulnerable())
...
def detect_vulnerabilities():
    ...
    if instructions:
        ...
        detect_reentrancy()
        detect_gas_hardcoded()
        detect_unexpected_ether()
...
def log_info():
    ...
    vulnerabilities = [#integer_underflow, integer_overflow, callstack, money_concurrency, time_dependency,
                       reentrancy, gas_hardcoded, unexpected_ether]
    if g_src_map:
